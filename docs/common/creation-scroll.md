...
**scroll-snap 미사용.** 브라우저 네이티브 스크롤을 그대로 활용한다.
- v1에서 `scroll-snap-type: y mandatory/proximity`를 시도했으나 자연스러운 느낌이 아니어서 제거
- `<main class="creation-main">`이 일반 블록 요소로 스크롤됨
- 커스텀 스크롤 래퍼(`overflow-y: scroll; height: 100vh`) 미사용
...
> 속도 튜닝 히스토리: 0.9s(너무 빠름) → 1.8s(너무 느림) → **1.2s(현재, 중간값)**

### 5-3. 진행 바 (Progress Bar)

화면 하단 고정 바(`position: fixed; bottom: 0`)로 진행률 표시.
...
| 결정 | 채택 | 기각 | 이유 |
|---|---|---|---|
| 스크롤 방식 | 네이티브 스크롤 | scroll-snap (mandatory/proximity) | snap이 자연스럽지 않음, 브라우저 기본 스크롤이 가장 편안 |

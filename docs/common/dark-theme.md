...
| 토글 UI 위치 | 계정 메뉴 안 "테마" 항목 → 인라인 서브메뉴로 라이트/다크/시스템 라디오 노출 |
...
### 4.1 테마 토글 UI — 계정 메뉴 안 인라인 서브메뉴

- **위치:** `topNavAccountMenu` 안 가장 위. 계정 항목들과 separator 로 분리
- **마크업 구조:**
  - "테마" 토글 항목 (`<button class="top-nav-theme-menu-toggle">`) — 좌측 🎨 아이콘 + "테마" 라벨 + 우측 현재 선택 인디케이터 (☀️/🌙/🖥️) + chevron (▾)
  - 서브메뉴 (`<div class="top-nav-theme-submenu">`) — `role="group"` 안 3개 라디오 버튼 (`role="menuitemradio"`)
  - 라디오 항목 라벨은 **단축형** ("라이트", "다크", "시스템") — 시각 깔끔함과 정보 명시성 절충. 스크린리더는 `aria-label` 의 풀 라벨("라이트 모드", "다크 모드", "시스템 따름") 로 읽음
- **상호작용:**
  - "테마" 항목 클릭 → 서브메뉴 펼침/접기 토글, chevron 180° 회전 애니메이션
  - 라디오 항목 클릭 → 즉시 적용 + 서브메뉴 닫힘 + 계정 메뉴 닫힘 (1 액션 마무리)
  - 계정 메뉴가 외부 클릭/Escape 로 닫히면 서브메뉴도 함께 접힘 (MutationObserver 동기화)
- **펼침 애니메이션:** `max-height: 0 → 200px` 220ms ease (CSS 만으로 처리, JS 의존성 없음)
- **현재 선택 표시:**
  - "테마" 항목 우측 — 현재 적용 모드 아이콘 (메뉴 닫힌 상태에서도 인지 가능)
  - 서브메뉴 내 — 좌측 체크 (`✓`) + 액센트 색상 + bold (`is-checked` 클래스 + `aria-checked="true"`)
- **시스템 따름 보조 텍스트:** "시스템 따름" 라디오 항목에 현재 OS 적용 모드 명시 — 예: `시스템 따름 (현재 다크)`
- **접근성:**
  - 토글 항목: `aria-haspopup="true"`, `aria-expanded`, `aria-controls="topNavThemeSubmenu"`
  - 라디오 그룹: `role="group"`, `aria-label="테마 선택"`
  - 라디오 항목: `role="menuitemradio"`, `aria-checked`
  - 키보드: 기존 계정 메뉴 동작과 동일 (Tab, Enter, Space, Escape)

#### 4.1.1 트레이드오프 — 인지된 한계

| 항목 | 영향 | 대응 |
|---|---|---|
| `hideTopNavAccount=true` 페이지에서 토글 미노출 | `my-memo.html` 등에서 테마 변경 불가 | 사용자가 일반 페이지에서 설정 → `localStorage` 영속화로 모든 페이지 적용 (해당 페이지에서 직접 변경만 불가) |
| 클릭 횟수 (3-click): 계정 → 테마 → 라디오 | 빠른 토글 대비 부담 | 다크 모드는 빈번한 변경 옵션이 아니므로 허용 가능 |
| 모달 대비 화면 점유 | 메뉴 안 인라인 펼침이라 dialog 보다 가벼움 | 펼침 애니메이션으로 명확한 시각 피드백 |

이 한계를 알고도 "메뉴 안 명시적 라디오 그룹" 의 인지성/확장성 가치를 우선해 채택.
...
| 키보드 | 토글 버튼 `Tab` 으로 도달, `Enter`/`Space` 로 다음 상태 순환, focus ring 양 테마 모두 가시 |
| 스크린리더 | `aria-label` 동적 갱신으로 현재 상태와 다음 동작 안내 (예: `테마: 다크 (클릭하여 시스템으로 변경)`) |
...
| 토글 UI | GNB 단일 버튼 (3-state 순환) | 계정 메뉴 내 3 항목 / 별도 드롭다운 | `hideTopNavAccount=true` 페이지에서도 노출 보장, 1-click 토글, 화면 점유 최소 |
...
- [ ] 토글 버튼 키보드 접근 (`Tab` 도달 + `Enter`/`Space` 순환)
- [ ] 스크린리더로 현재 상태와 다음 동작 인지 가능 (`aria-label` 동적 갱신)

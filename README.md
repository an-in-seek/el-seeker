...
| 기능           | 경로                       | 설명                                      |
|--------------|--------------------------|-----------------------------------------|
| 번역본 목록 조회    | `/web/bible/translation` | 성경 번역본 리스트를 조회하여 화면에 렌더링함               |
| 책 목록 페이지 진입  | `/web/bible/book`        | 선택한 번역본에 따라 성경 책 정보를 보여줄 페이지 (JS 처리 예상) |
| 장 목록 페이지 진입  | `/web/bible/chapter`     | 선택한 책에 따라 장 정보를 보여줄 페이지 (JS 처리 예상)      |
| 절 목록 페이지 진입  | `/web/bible/verse`       | 선택한 장에 따라 절 정보를 보여줄 페이지 (JS 처리 예상)      |
| 구절 검색 페이지 진입 | `/web/bible/search`      | 키워드 기반 성경 구절 검색 페이지 렌더링                 |

> 📌 `showTranslations()` 메서드 외에는 현재 백엔드에서 데이터를 모델에 주입하지 않고, 프론트엔드(JavaScript 등)에서 API를 호출해 동적으로 처리할 것으로 추정됩니다.
